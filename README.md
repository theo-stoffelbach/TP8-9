...
```bash
# Customers
docker exec -it customers_service python manage.py loaddata catalog_import_10k.json
...
```bash
# Copier le script dans le conteneur BI
...
```
                    ...
                    └──────┬──────┘
                           │
    ┌─────────────┐   ┌────┴──────────┐   ┌────────────────┐
    │dim_customer │───│fact_order_    │───│  dim_product   │
    └─────────────┘   │   lines       │   └───────┬────────┘
                      └──────┬────────┘           │
                             │            ┌───────┴──────┐
                             │            │  dim_category│
...
**Tables :**
- `dim_date` — dimension temporelle (year, quarter, month, month_name, day, day_name)
- `dim_customer` — clients avec pays et ville (10 000 clients)
- `dim_category` — catégories de produits (50 catégories)
- `dim_product` — produits avec `category_name` dénormalisé (94 990 produits)
- `fact_order_lines` — table de faits (1 ligne = 1 ligne de commande, 152 317 lignes)

**Vues pour Superset :**
- `vw_fact_order_lines_complete` — faits + toutes les dimensions jointes
...
- `vw_sales_by_customer` — ventes agrégées par client
- `vw_top_products` — top produits par CA et quantité
...
| Dashboard | Statut | Charts inclus |
|-----------|--------|---------------|
| **Vue globale** | ✅ | CA total, Nombre de commandes, Produits vendus, Panier moyen, Évolution CA |
| **Analyse produits** | ✅ | Top 10 produits par CA, Top 10 par quantité, CA par catégorie, Quantité par catégorie |
| **Analyse clients** | ❌ | À créer |
| **Analyse géographique** | ❌ | À créer |

### Partager les dashboards

**Exporter tes dashboards :**
```bash
bash analytics/export-superset.sh
```
Puis commit le fichier `analytics/superset_export.zip`.

**Importer les dashboards (pour tes collègues) :**
```bash
